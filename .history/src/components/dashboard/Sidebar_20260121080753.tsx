    ...
    <div className="flex flex-col h-full bg-brand-blue p-6 shadow-2xl">
      ...
      <div className="pt-6 border-t border-white/10">
        <button
          ...
          <div className="p-2 bg-red-500/10 group-hover:bg-white/20 rounded-lg">
            <LogOut size={20} />
          </div>
...
const Sidebar = ({ isOpen, setIsOpen }: any) => {
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 w-72 transform transition-transform duration-500 lg:relative lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <SidebarContent
        pathname={pathname}
        user={user}
        logout={logout}
