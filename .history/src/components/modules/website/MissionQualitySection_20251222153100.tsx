...
const TypewriterTitle = ({ text }: { text: string }) => {
  ...
  const container = {
    ...
    exit: { opacity: 0, transition: { duration: 0.5 } }, // Fade out before restart
  };
        ...
        {features.map((feature, index) => (
          <motion.div
            ...
            <div className="relative h-full bg-white p-8 rounded-[2rem] shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border-t-4 border-brand-red">
              {/* DEFAULT BG: Subtle Gradient to prevent 'boring' look */}
              <div className="absolute inset-0 bg-gradient-to-b from-white to-gray-50/80 z-0" />

              {/* HOVER BG: Red Gradient Sweep */}
              <div className="absolute inset-0 bg-gradient-to-br from-brand-red to-red-900 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10" />

              {/* WATERMARK ICON (Visible by default now!) */}
              <feature.icon
                className="absolute -bottom-8 -right-8 w-40 h-40 text-brand-red/5 group-hover:text-white/10 transition-all duration-700 group-hover:rotate-12 transform z-0"
                strokeWidth={1}
              />
