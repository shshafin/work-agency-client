  ...
  return (
    <>
      ...
      <motion.header
        ...
        className={cn(
          "fixed z-40 w-full transition-all duration-500 flex items-center",
          // Adjusted: Height is now explicit to ensure vertical centering
          isScrolled
            ? "bg-white/95 backdrop-blur-md shadow-xl border-b border-gray-100 top-0 h-16 md:h-20"
        ...
        <div className="container mx-auto flex items-center justify-between px-4 lg:px-6">
          ...
          <nav className="hidden xl:flex items-center gap-6 2xl:gap-8">
            {navLinks.map((link) => (
              <Link
                ...
                <span
                  className={cn(
                    "text-[15px] font-bold tracking-tight uppercase transition-colors duration-300",
                    isScrolled
                      ? "text-gray-800"
          ...
          <div className="hidden xl:flex items-center shrink-0">
            <Link href="/contact">
              <PoshButton className="shadow-lg h-11 px-6 text-[11px] font-black uppercase tracking-widest bg-linear-to-r from-brand-red to-red-800 text-white hover:shadow-red-500/20 transition-all active:scale-95">
                Get Quote
              </PoshButton>
            </Link>
          </div>
