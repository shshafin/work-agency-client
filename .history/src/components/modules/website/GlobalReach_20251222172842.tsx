...
const AnimatedCounter = ({
  ...
  const inView = useInView(ref, { once: true }); // Play once when seen
  ...
  useEffect(() => {
    if (inView) {
      ...
      // Animate from 0 to 'to' over 2 seconds
      const animation = motion.animate(count, to, { duration: 2 });
        ...
        <motion.h2
          ...
          {letters.map((letter, index) => (
            <motion.span
              variants={child}
              key={index}
              // Color the word "World" (last 5 letters approx) Red
    ...
    <SectionWrapper className="bg-white overflow-hidden relative border-t border-gray-100">
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:items-center">
        ...
        <div className="lg:w-2/3 relative h-[400px] md:h-[500px] w-full bg-slate-50 rounded-3xl border border-gray-200 overflow-hidden shadow-inner group/map">
          ...
          {/* 2. DOTTED OVERLAY (CSS Pattern) */}
          {/* This creates the "tech/dotted" look over the real image */}
          <div
