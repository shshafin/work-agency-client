...
// --- REUSABLE TYPEWRITER COMPONENT (Fixed for Mobile) ---
const TypewriterTitle = ({ text }: { text: string }) => {
  ...
  return (
    // UPDATED: Added min-h-fit and flex-wrap to handle multi-line text on mobile
    <div className="min-h-[80px] md:min-h-[70px] flex items-center flex-wrap">
      <AnimatePresence mode="wait">
        <motion.h2
          key={key}
          // UPDATED: text-3xl for mobile, sm:text-4xl, md:text-6xl for desktop
          className="text-3xl sm:text-4xl md:text-6xl font-bold text-white leading-tight break-words"
          variants={container}
          initial="hidden"
          animate="visible"
          exit="exit">
          {letters.map((letter, index) => (
            <motion.span
              variants={child}
              key={index}
              // Logic: "Inside The " is white, "Powerhouse" (starts at index 11) is Red
...
const FactoryInsight = () => {
  ...
  return (
    <SectionWrapper className="relative bg-neutral-950 overflow-hidden py-16 md:py-24">
      {/* =========================================
          1. BACKGROUND EFFECTS
      ========================================= */}

      {/* Layer 1: Radial Red Glow */}
      ...
      <div className="absolute top-20 right-20 w-64 h-64 bg-brand-yellow/5 rounded-full blur-[80px] animate-pulse pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-16 gap-6 md:gap-8">
          <div className="max-w-full">
            ...
            {/* CONTINUOUS ANIMATED HEADING */}
            <TypewriterTitle text="Inside The Powerhouse" />
          </div>

          <p className="text-gray-400 max-w-sm text-sm md:text-base leading-relaxed border-l-2 border-brand-red/50 pl-6">
        ...
        <div
          ...
          <div className="absolute top-4 right-4 md:top-8 md:right-8">
            <ArrowRight className="text-white/60 w-6 h-6 md:w-8 md:h-8 -rotate-45 group-hover:rotate-0 group-hover:text-brand-yellow transition-all duration-500" />
          </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative w-20 h-20 md:w-32 md:h-32 flex items-center justify-center">
              ...
              {/* Center Icon */}
              <div className="w-10 h-10 md:w-16 md:h-16 bg-brand-yellow rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(255,215,0,0.4)] group-hover:scale-110 transition-transform duration-300">
                <Play
                  size={20}
          ...
          {/* BOTTOM INFO */}
          <div className="absolute bottom-0 left-0 w-full p-4 md:p-8 bg-gradient-to-t from-black/90 to-transparent flex justify-between items-end transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
            <div>
