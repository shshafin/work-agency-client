...
// --- 1. TYPEWRITER HEADER (FIXED RESPONSIVENESS) ---
const TypewriterTitle = ({ text }: { text: string }) => {
  ...
  // FIX: Split by words so they stay together but break lines on mobile
  ...
  return (
    <div className="min-h-15 flex justify-center items-center px-4">
      <AnimatePresence mode="wait">
        <motion.h2
          key={key}
          // ADDED: flex-wrap, justify-center, and gap-x-2 to handle wrapping
          ...
          {words.map((word, wordIndex) => (
            // Wrap words in a span to keep letters together
            <span
              key={wordIndex}
...
const FeaturedCollections = () => {
  return (
    <SectionWrapper className="bg-white overflow-hidden relative">
      ...
      {/* --- BENTO GRID --- */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-162.5 px-4 md:px-0">
        ...
        <TiltCard
          ...
          <div className="relative h-full w-full">
            ...
            />
            {/* OVERLAY: Using your newer Tailwind syntax */}
