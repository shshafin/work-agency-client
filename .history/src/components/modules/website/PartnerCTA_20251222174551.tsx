...
const PartnerCTA = () => {
  ...
  // Move text left and right based on scroll
  const xLeft = useTransform(scrollYProgress, [0, 1], ["0%", "-20%"]);
  const xRight = useTransform(scrollYProgress, [0, 1], ["-20%", "0%"]);

  return (
    <SectionWrapper className="relative bg-brand-red overflow-hidden py-32 flex flex-col items-center justify-center min-h-[600px]">
      {/* =========================================
          BACKGROUND MARQUEE TEXT (Giant)
      ========================================= */}
      ...
      {/* Decorative Glow */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-150 h-[300px] bg-brand-yellow/20 rounded-full blur-[100px] pointer-events-none" />
    </SectionWrapper>
  );
};
