...
const DepartmentsSection = () => {
  ...
  // 🌀 EFFECT: Move the highlight every 6 seconds (much slower)
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % departments.length);
    }, 6000); // Speed of the loop - increased to 6000ms for much slower animation
        ...
        {departments.map((dept, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={cn(
                  "relative h-full p-8 rounded-2xl transition-all duration-[800ms] border overflow-hidden",
                ...
                <div
                  className={cn(
                    "absolute inset-0 bg-linear-to-r from-transparent via-white/20 to-transparent skew-x-12 transition-transform duration-[2000ms]",
