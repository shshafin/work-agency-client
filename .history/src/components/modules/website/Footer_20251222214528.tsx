...
import {
  ...
  ArrowRight,
  Globe,
  // Added extra icons for full compatibility
  Trello as TikTok, // TikTok doesn't exist in Lucide yet, usually Trello or a custom SVG is used, or "Music"
  Music,
  Disc as Reddit,
  Pin as Pinterest,
} from "lucide-react";
...
const Footer = () => {
  ...
  // This maps every field from your ISiteSetting.socialLinks interface
  const socialIcons = [
    ...
    { Icon: Music, link: settings?.socialLinks?.tiktok }, // Using Music icon for TikTok
    { Icon: Pinterest, link: settings?.socialLinks?.pinterest },
    { Icon: Reddit, link: settings?.socialLinks?.reddit },
  ].filter((social) => social.link); // Dynamically only show what's entered in the DB
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-6">
            ...
            {/* Dynamic Social Icons - Now handles all platforms */}
            <div className="flex flex-wrap items-center gap-3">
              {socialIcons.map(({ Icon, link }, i) => (
                <a
                  ...
                  className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-brand-red hover:text-white transition-all duration-300 group shadow-sm"
                  title={link}>
                  <Icon
                    size={18}
              ...
              {[
                ...
                <li
                  ...
                  <Link
                    ...
                    <ArrowRight
                      size={14}
                      className="opacity-0 group-hover:opacity-100 transition-all text-brand-red"
                    />
                    {item.label}
                  </Link>
                </li>
