...
// --- Looping Typewriter Component (FIXED RESPONSIVENESS) ---
const TypewriterTitle = ({ text }: { text: string }) => {
  ...
  // FIX: Split text into words to handle wrapping correctly
  ...
  return (
    <div className="min-h-15 flex justify-center items-center px-4">
      <AnimatePresence mode="wait">
        <motion.h2
          key={key}
          // UPDATED: text-3xl for mobile, flex-wrap, justify-center, gap-x-3
          className="text-3xl md:text-5xl font-bold text-brand-red mb-4 flex flex-wrap justify-center gap-x-3"
          variants={container}
          initial="hidden"
          animate="visible"
          exit="exit">
...
const MissionQualitySection = () => {
  return (
    <SectionWrapper className="relative overflow-hidden bg-gray-50/50">
      {/* --- HEADER --- */}
      <div className="text-center mb-16 max-w-3xl mx-auto relative z-10 px-4">
        ...
        {features.map((feature, index) => (
          <motion.div
            ...
            <div className="relative h-full bg-white p-8 rounded-[2rem] shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border-t-4 border-brand-red">
              {/* LAYER 0: Default BG */}
              <div className="absolute inset-0 bg-linear-to-b from-white to-gray-50/80 z-0" />

              {/* LAYER 1: Hover Red Gradient */}
              <div className="absolute inset-0 bg-linear-to-br from-brand-red to-red-900 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0" />
