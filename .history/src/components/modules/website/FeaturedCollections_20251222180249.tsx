...
// --- 1. UPDATED TYPEWRITER HEADER (Responsive Fix) ---
const TypewriterTitle = ({ text }: { text: string }) => {
  ...
  return (
    <div className="min-h-[60px] flex justify-center items-center px-4">
      <AnimatePresence mode="wait">
        <motion.h2
          key={key}
          // Changed text-4xl to text-3xl for mobile
          // Added flex & flex-wrap to allow breaking lines
          // Added gap-x-3 for spacing between words
          ...
          {words.map((word, wordIndex) => (
            // Wrap each word in a span so words stay together but lines break between them
            <span
              key={wordIndex}
...
const FeaturedCollections = () => {
  return (
    <SectionWrapper className="bg-white overflow-hidden relative">
      ...
      {/* --- BENTO GRID --- */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[650px] px-4 md:px-0">
        ...
        <TiltCard
          ...
          <div className="relative h-full w-full">
            ...
            />
            {/* OVERLAY */}
            <div className="absolute inset-0 bg-gradient-to-t from-brand-red/90 via-transparent to-transparent opacity-50 group-hover:opacity-90 transition-opacity duration-500" />
