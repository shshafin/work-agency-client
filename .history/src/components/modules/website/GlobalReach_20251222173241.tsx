        ...
        <motion.h2
          ...
          {letters.map((letter, index) => (
            <motion.span
              variants={child}
              key={index}
              // UPDATED: "The " is 4 chars. "World" starts at index 4.
              className={index >= 4 ? "text-brand-red" : "text-gray-900"}>
              {letter === " " ? "\u00A0" : letter}
            </motion.span>
          ))}
        </motion.h2>
...
// --- DATA: LOCATIONS ---
// Adjusted 'left' slightly to prevent overlapping tooltips since they are always open now
const locations = [
  { country: "UK", details: "Southampton", top: "25%", left: "46%" },
  { country: "Germany", details: "Hamburg Hub", top: "30%", left: "53%" },
  { country: "Netherlands", details: "Rotterdam", top: "25%", left: "51%" }, // Shifted slightly
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:items-center">
        ...
        <div className="lg:w-2/3 relative h-[450px] md:h-[550px] w-full bg-slate-50 rounded-3xl border border-gray-200 overflow-hidden shadow-inner group/map">
          ...
          {locations.map((loc, index) => (
            <div
              key={index}
              // Added z-index behavior: hover brings to front
              ...
              {/* The Dot */}
              <div className="relative w-4 h-4 bg-brand-red border-2 border-white shadow-lg rounded-full z-10" />

              {/* TOOLTIP (ALWAYS VISIBLE NOW) */}
              {/* Removed opacity-0 and group-hover classes */}
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-32 md:w-40 bg-white/90 backdrop-blur-sm p-2 md:p-3 rounded-lg shadow-xl border border-gray-100 z-20">
