...
const TypewriterTitle = ({ text }: { text: string }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setKey((prev) => prev + 1);
    }, 8000); // Slower cycle for the title
  ...
  const container: Variants = {
    ...
    visible: (i = 1) => ({
      opacity: 1,
      transition: { staggerChildren: 0.05, delayChildren: 0.1 * i }, // Slower letter stagger
    }),
...
const DepartmentsSection = () => {
  ...
  // 🌀 SLOWER EFFECT: Increased from 2000 to 4000
  useEffect(() => {
        ...
        {departments.map((dept, index) => {
          ...
          return (
            <motion.div
              ...
              <div
                className={cn(
                  "relative h-full p-8 rounded-2xl transition-all duration-1000 border overflow-hidden", // Increased duration
                  isActive
                ...
                {/* 1. SLOWER FLASH EFFECT */}
                <div
                  className={cn(
                    "absolute inset-0 bg-linear-to-r from-transparent via-white/20 to-transparent skew-x-12 transition-transform duration-1500",
                ...
                <div className="flex justify-between items-start mb-6 relative z-10">
                  <span
                    className={cn(
                      "text-5xl font-black font-sans tracking-tighter transition-colors duration-1000",
                  ...
                  <div
                    className={cn(
                      "p-3 rounded-xl border transition-all duration-700 shadow-lg",
                      isActive
                ...
                {/* 4. SLOWER PROGRESS BAR (Matches 4000ms interval) */}
                <div className="absolute bottom-0 left-0 w-full h-1 bg-white/10">
                  <div
                    className={cn(
                      "h-full bg-brand-yellow transition-all ease-linear",
                      isActive ? "w-full duration-4000" : "w-0 duration-0"
                    )}
                  />
                </div>
              </div>
