...
import { Bold, Italic, List, ListOrdered, Code, Heading1, Heading2, Quote, SeparatorHorizontal, Strikethrough, Redo, Undo } from 'lucide-react';
...
const MenuBar = ({ editor }: { editor: any }) => {
  ...
  const buttons = [
    ...
    { icon: SeparatorHorizontal, action: () => editor.chain().focus().setHorizontalRule().run(), isActive: false },
    { icon: Undo, action: () => editor.chain().focus().undo().run(), isActive: false, disabled: !editor.can().undo() },
    { icon: Redo, action: () => editor.chain().focus().redo().run(), isActive: false, disabled: !editor.can().redo() },
  ];
        ...
        return (
          <button
            ...
            className={cn(
              ...
              item.isActive ? "bg-black text-white" : "text-gray-600 hover:bg-gray-200 disabled:opacity-50"
            )}
            type="button"
          >
...
const GSLTipTapEditor = ({ name, label, placeholder }: GSLTipTapEditorProps) => {
        ...
        const editor = useEditor({
          ...
          editorProps: {
            attributes: {
                ...
                class: 'prose max-w-none focus:outline-none p-4 min-h-[300px] border border-transparent' 
            }
          }
        }, [field.value]); 
