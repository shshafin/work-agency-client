...
const Card = ({
  ...
  // ব্যাকএন্ডের এনামের সাথে সেক্টর নাম ম্যাচ করানো
  const querySector = item.title === "General Labour" ? "Others" : item.title;
    ...
    <motion.div
      ...
      <div
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
          <div className="absolute -inset-full animate-[spin_5s_linear_infinite] bg-[conic-gradient(from_0deg,transparent,white,transparent)] opacity-20" />
        </div>
        ...
        <div
          ...
          <Link
            href={`/jobs?sector=${querySector}`}
            className="inline-flex items-center gap-2 text-white font-bold group/link uppercase text-[10px] tracking-[0.2em]">
            View Opportunities
            <div className="w-8 h-0.5 bg-white/20 group-hover/link:w-12 group-hover/link:bg-white transition-all duration-300" />
            <ArrowRight
              size={16}
  ...
  return (
    <section className="py-32 bg-[#020617] relative overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        />
        <div className="absolute top-0 left-1/4 w-125 h-125 bg-blue-600/20 rounded-full blur-[120px] animate-pulse" />
        <div className="absolute bottom-0 right-1/4 w-125 h-125 bg-orange-500/10 rounded-full blur-[120px]" />
      </div>
