...
const Advantages = () => {
  ...
  return (
    <section className="py-24 bg-brand-light relative overflow-hidden">
      ...
      <div className="absolute top-10 left-10 text-[15vw] font-black text-brand-blue/[0.02] select-none pointer-events-none leading-none">
        ADVANTAGE
      </div>

      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24 items-start">
          {/* --- Left: Immersive Display Area --- */}
          <div className="w-full lg:w-1/2 lg:sticky lg:top-32">
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              whileInView={{ opacity: 1, x: 0 }}
              className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-white border border-brand-blue/5 shadow-sm mb-8">
            ...
            <h2 className="text-5xl md:text-8xl font-black text-brand-blue tracking-tighter leading-[0.85] mb-12">
              Why <br /> Partners <br />
          ...
          {/* --- Right: Interactive Selector Stack --- */}
          <div className="w-full lg:w-1/2 space-y-4">
            {advantages.map((item, index) => (
              <button
                ...
                className={`w-full text-left p-8 rounded-[2rem] border transition-all duration-500 group flex items-center justify-between ${
                  activeIndex === index
                    ? "bg-white border-brand-yellow shadow-xl translate-x-4"
            ...
            <motion.div
              ...
              className="pt-8">
              <button className="w-full p-8 bg-brand-yellow rounded-[2rem] flex items-center justify-between group overflow-hidden relative">
                <div className="relative z-10">
                  <h3 className="text-2xl font-black text-brand-blue">
                    Start Your Journey
                  </h3>
                  <p className="text-brand-blue/60 text-sm font-bold uppercase tracking-widest">
                    Let&apos;s build excellence together
                  </p>
                </div>
                <div className="relative z-10 w-16 h-16 bg-brand-blue rounded-full flex items-center justify-center text-white group-hover:scale-110 transition-transform">
                  <ArrowRight />
                </div>
                <div className="absolute top-0 right-0 w-32 h-full bg-white/10 skew-x-[30deg] translate-x-10" />
              </button>
            </motion.div>
          </div>
        </div>
