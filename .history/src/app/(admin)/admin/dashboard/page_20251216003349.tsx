...
import { Package, Users, Mail, FileText, Activity, Clock } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { getAllProducts } from "@/services/ProductService";
import { getAllContacts } from "@/services/ContactService";
import { getAllUsers } from "@/services/UserService";
import { getAllResources } from "@/services/ResourceService";
import { IFaq } from "@/types/faq.types"; // Assuming you have this
import { getAllFaqs } from "@/services/FaqService"; // Assuming you have this
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [productsRes, messagesRes, usersRes, resourcesRes, faqsRes] = await Promise.all([
          getAllProducts(),
          getAllContacts(),
          getAllUsers(),
          getAllResources(),
  ...
  const statCards = [
    { title: "Total Products", value: stats.products, icon: Package, color: "text-blue-600", bg: "bg-blue-50" },
    { title: "New Messages", value: stats.messages, icon: Mail, color: "text-purple-600", bg: "bg-purple-50" },
    { title: "Active Users", value: stats.totalUsers, icon: Users, color: "text-green-600", bg: "bg-green-50" },
    { title: "PDF Resources", value: stats.resources, icon: FileText, color: "text-orange-600", bg: "bg-orange-50" },
  ];
  ...
  const recentActivity = [
    ...
    { time: '5 mins ago', description: 'Moderator created a new Product: "Metal Alloy X".' },
    { time: '1 hour ago', description: 'Admin deleted User ID #456.' },
    { time: '3 hours ago', description: 'New contact message received from Jane Doe.' },
    { time: 'Yesterday', description: 'System backup initiated.' },
  ];


  if (loading) return <div className="p-10 text-center text-lg text-gray-500">Loading Dashboard Data...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* --- Section 1: Stat Cards (4-Column Grid) --- */}
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={activityData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  />
                  <Legend />
                  <Bar dataKey="count" name="Activity Count" radius={[4, 4, 0, 0]}>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>A log of key actions performed by admins.</CardDescription>
        </CardHeader>
        <CardContent>
          <ol className="relative border-l border-gray-200 ml-4">                  
            {recentActivity.map((activity, index) => (
              <li key={index} className="mb-4 ml-6">
                <span className="absolute flex items-center justify-center w-3 h-3 bg-gray-200 rounded-full -left-1.5 ring-4 ring-white">
                </span>
      ...
      {/* Existing Welcome Section - kept below the data */}
      <div className="bg-white rounded-lg border p-8 text-center mt-8 shadow-inner">
         ...
         <p className="text-gray-500 max-w-lg mx-auto mt-2">
           Select a module from the sidebar to manage your website content. 
           Your dashboard provides real-time stats and system logs for monitoring.
         </p>
      </div>
    </div>
  );
