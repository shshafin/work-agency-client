        ...
        <CardContent>
          <GSLForm form={form} onSubmit={onSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <GSLInput name="name" label="Product Name" required />
              <GSLSelect name="category" label="Category" options={categories} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <GSLInput name="price" label="Price" type="number" />
              <div className="flex flex-col gap-3 mt-2">
                 <label className="text-sm font-medium">Featured Status</label>
                 <div className="flex items-center gap-2 border p-3 rounded-md">
                    <Checkbox 
                      checked={form.watch("isFeatured")}
                      onCheckedChange={(val) => form.setValue("isFeatured", val as boolean)}
                    />
                    <span className="text-sm text-gray-500">Show on Home Page</span>
                 </div>
              </div>
            </div>

            <GSLTextarea name="description" label="Description" />
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {existingImages.map((img, idx) => (
                    <div key={idx} className="relative h-20 w-20 rounded-md overflow-hidden border">
                      <Image src={img} alt="Current" fill className="object-cover" />
                    </div>
                  ))}
                </div>
                <p className="text-xs text-gray-500">New uploads will be added to existing ones.</p>
              </div>
            )}

            <GSLImageUpload name="images" label="Upload New Images (Appends to existing)" />

            <div className="flex justify-end gap-4">
               <Button type="button" variant="outline" onClick={() => router.back()}>Cancel</Button>
               <Button type="submit" className="bg-black">Update Product</Button>
            </div>
