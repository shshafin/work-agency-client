...
import { Edit, Plus, Trash, Shield, ShieldCheck, Search, User, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function UserListPage() {
  ...
  const getRoleBadge = (role: TUserRole) => {
    switch (role) {
      case "super_admin":
        return <Badge className="bg-purple-100 text-purple-700 border-purple-200 hover:bg-purple-200"><Shield className="w-3 h-3 mr-1"/> Super Admin</Badge>;
      case "admin":
        return <Badge className="bg-blue-100 text-blue-700 border-blue-200 hover:bg-blue-200"><ShieldCheck className="w-3 h-3 mr-1"/> Admin</Badge>;
      default:
        return <Badge variant="outline" className="text-gray-600"><User className="w-3 h-3 mr-1"/> Moderator</Badge>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <TableSkeleton /> : <GSLTable columns={columns} data={paginatedUsers} />}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmDelete} className="bg-red-600">Delete User</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
