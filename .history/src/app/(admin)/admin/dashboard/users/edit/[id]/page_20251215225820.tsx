...
// 🛠️ Updated Schema: Password is optional for updates
...
export default function EditUserPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (res.success) {
          ...
          form.reset({
            name: data.name,
            role: data.role,
            password: "", // Always reset password to empty on load
            image: data.image,
          });
        }
      } catch (error) {
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      // 🧠 Logic: Only send password if admin typed one
      ...
      if (values.password && values.password.length > 0) {
        if (values.password.length < 6) {
          toast.error("Password must be at least 6 chars", { id: toastId });
          return;
        }
          ...
          <GSLForm form={form} onSubmit={onSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="md:col-span-1">
                 <GSLSingleImageUpload name="image" label="Profile Picture" />
              </div>

              {/* Right: Inputs */}
              <div className="md:col-span-2 space-y-4">
                <GSLInput name="name" label="Full Name" />
                ...
                <div className="p-4 bg-gray-50 border rounded-md mt-4">
                  <h3 className="text-sm font-semibold mb-2 text-gray-700">Change Password</h3>
                  <GSLInput 
                    name="password" 
                    label="New Password" 
