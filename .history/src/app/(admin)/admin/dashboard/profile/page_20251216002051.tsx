...
import GSLSingleImageUpload from "@/components/core/GSLForm/GSLSingleImageUpload"; // 🆕 Import this
...
export default function ProfilePage() {
  ...
  // 🛠️ Updated Schema: Include image
  const form = useForm({
    resolver: zodResolver(
      z.object({
        name: z.string().min(1, "Name is required"),
        image: z.any().optional(), // 🆕 Allow image
      })
    ),
    defaultValues: {
      name: "",
      ...
      getSingleUser(user.userId)
        .then((res) => {
          if (res.success) {
            ...
            // Reset form with name AND existing image URL
            form.reset({
              name: res.data.name,
  ...
  const handleUpdate = async (values: any) => {
    ...
    try {
      ...
      // 2. Send File (Only if a new file is selected)
      ...
      if (res.success) {
        ...
        setProfileData((prev: any) => ({
          ...prev,
          name: values.name,
          // If the backend returns the new image URL, use it. Otherwise rely on refresh.
          // Usually better to reload or update if backend sends back the updated user object.
          image: res.data?.image || prev.image,
        }));

        // Optional: Force reload to update Navbar avatar if context doesn't auto-update
            ...
            <GSLForm
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <GSLInput
                  ...
                  label="Full Name"
                />
                {/* Read Only Email Field for visual confirmation */}
                <div className="space-y-2">
                  <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                    Email Address
                  </label>
                  <div className="flex h-10 w-full rounded-md border border-input bg-gray-100 px-3 py-2 text-sm text-gray-500 cursor-not-allowed">
                    {profileData?.email}
                  </div>
                </div>
              </div>
