...
const MyProfile = () => {
  ...
  const onUpdateProfile = async (data: any) => {
    setLoading(true);
    try {
      // টোকেন থেকে userId নিয়ে আপডেট কল করছি
      const res = await UserService.updateUser(user.userId, data);

      if (res.success) {
        // Zustand স্টোর আপডেট করছি যাতে সাইডবার/হেডারে নাম বদলে যায়
        // ব্যাকএন্ড থেকে আসা নতুন ডেটা (res.data) সেট করছি
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        {/* Profile Card (No Image/Upload) */}
        <div className="lg:col-span-1">
          <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm text-center">
            <div className="w-24 h-24 bg-brand-blue rounded-3xl flex items-center justify-center text-brand-yellow text-4xl font-black mx-auto mb-6 shadow-xl">
              {user?.name?.charAt(0).toUpperCase()}
            </div>
            <h3 className="text-xl font-black text-brand-blue truncate">
              {user?.name}
            </h3>
            <span className="text-[10px] font-black text-brand-yellow bg-brand-blue px-4 py-1.5 rounded-full uppercase inline-block mt-3 tracking-widest">
              {user?.role?.replace("_", " ")}
            </span>
          </div>
        </div>
