...
const TeamCard = ({ member }: { member: ITeamMember }) => {
  return (
    <Link href={`/team/${member._id}`} className="block h-full">
      <motion.div
  ...
  useEffect(() => {
    const fetchTeam = async () => {
      try {
        ...
        const sortedList = [...memberList].sort((a: ITeamMember, b: ITeamMember) => {
      ...
      <SectionWrapper className="py-0">
        {loading ? (
          ...
          <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
            <h3 className="text-xl font-bold text-gray-400">No team members found.</h3>
          </div>
        ) : (
          <motion.div
